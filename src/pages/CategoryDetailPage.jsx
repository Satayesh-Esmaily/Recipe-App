...
function CategoryDetailPage() {
  ...
  const categoryRecipes = useMemo(() => {
    const decoded = decodeURIComponent(cuisine || "");
    return recipes.filter((recipe) => recipe.cuisine === decoded);
  }, [recipes, cuisine]);
    ...
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-10 px-6 py-16">
      <header className="flex flex-col gap-4 rounded-3xl border border-[var(--border)] bg-[var(--surface)]/80 p-8">
        ...
        <p className="max-w-2xl text-sm text-[var(--muted)]">
          Browse recipes in this cuisine.{" "}
          <Link to="/categories" className="text-[var(--text)] underline">
            Back to categories
          </Link>
        </p>
      </header>
      ...
      {!loading && !error && categoryRecipes.length === 0 && (
        <div className="rounded-3xl border border-dashed border-[var(--border)] bg-[var(--surface)]/80 p-10 text-center text-[var(--muted)]">
